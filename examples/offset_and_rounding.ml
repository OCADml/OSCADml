...
(** {{!OSCADml.Poly2.offset} [Poly2.offset]}, and {{!OSCADml.Path2.offset}
...
(** More fine-grained control over 2d {i and} 3d path roundovers can be found in the
    {{!OSCADml.Path2.Round} [Path2.Round]} and {{!OSCADml.Path3.Round}
    [Path3.Round]} modules respectively. For this example, we'll use the
    {{!OSCADml.Path2.Round.circles} [Path2.Round.circles]} constructor to
...
(** Then we'll apply our {{!OSCADml.Path2.Round.t} [Path2.Round.t]} [shape_spec]
    with {{!OSCADml.Path2.roundover} [Path2.roundover]}, with [fn] segments per
